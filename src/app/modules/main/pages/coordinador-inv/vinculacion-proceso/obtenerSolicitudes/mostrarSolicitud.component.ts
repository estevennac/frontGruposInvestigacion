...
export class VinculacionForCoor implements OnInit {
  ...
  validarSolicitud() {
    if(this.link.estado==='e'){
      this.linkDataUpdate = {
        idUser: this.link.idUser,
        idGrupoInv: this.link.idGrupoInv,
        estado:'d'
      }
      this.linkService.update(this.link.idVinculacion, this.linkDataUpdate).subscribe(response=> {
        this.router.navigate(["main/sol-vinc-c"])
        
            })
    }
    if(this.link.estado==='d'){
      this.linkDataUpdate = {
        idUser: this.link.idUser,
        idGrupoInv: this.link.idGrupoInv,
        estado:'v'
      ...
      this.linkService.update(this.link.idVinculacion, this.linkDataUpdate).subscribe(response=> {
        this.router.navigate(["main/sol-vinc-d"])
