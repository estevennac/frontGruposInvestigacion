...
export class ActaTecComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.validacion=navigationState.validacion
  }
  onDrop(event: any) {
  ...
  onSubmit() {
    if (this.selectedFile) {
      ...
      const customFileName =  `Resolucion_Vinc_Sol_Grupo${groupId}_${year}-${month}-${day}_${hours}-${minutes}-${seconds}`;
      ...
      this.annexesService.createAnnexesForm(annexesData).subscribe(
        () => {
          ...
          setTimeout(() => {
            this.router.navigateByUrl('main/solicitudes-V');
          }, 1000); 
        },
        (error) => {
  ...
  goBack() {
    ...
    this.router.navigate(["main/solicitud-vin-t"],{state:{link}});
  }
