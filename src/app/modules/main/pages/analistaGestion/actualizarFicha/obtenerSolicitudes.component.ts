...
export class ActualizacionForAnaComponent implements OnInit {
    ...
    filtrarSolicitudes() {
        this.linkService.getByState("a").subscribe((data)=>{
    ...
    validar(id: number) {
        ...
        this.linkService.getById(id).subscribe(
            (link: LinkForm) => {
                this.router.navigate(['main/solicitud-vin-a'], { state: { link } });
            },
