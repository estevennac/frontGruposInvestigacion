...
export class SolicitudesForVicerectorIComponent implements OnInit {
    ...
    filtrarSolicitudes() {
        this.solicitudesFiltradas = this.creationReq.filter((solicitud) => solicitud.estado === 'V');
...
validar(id:number)
{
    ...
    this.solicitudService.getById(id).subscribe(
        (creationReqForm: CreationReqForm)=>{
            this.router.navigate(['main/inscripcionV'],{state:{creationReqForm}});
        },
