...
export class ActaVicComponent implements OnInit {
  ...
  onSubmit() {
    if (this.selectedFile) {
      ...
      const customFileName =  `Resolucion_Inscripcion_Grupo${groupId}_${year}-${month}-${day}_${hours}-${minutes}-${seconds}`;
      ...
      this.annexesService.createAnnexesForm(annexesData).subscribe(
        () => {
          ...
          setTimeout(() => {
            this.router.navigateByUrl('main/sol-vinc-v');
          }, 1000); 
        },
        (error) => {
